...
you need to install a library called PyDrive (installation method Is in the notebook)
Paste your dataset or file in the colab notebooks folder in drive.
right click on it and click on get link
paste the link anywhere (preferrabally in notepad) & extract the file ID
the file ID can be obtained from the link. The file ID is alphanumeric characters between /d/ and /view?usp=sharing
Paste File ID here
You can replace the intents.json filename to whatever you want.
...
Seems familiar? If you noticed , this looks similar to nested dictionary in python. That’s how JSON is formatted. Now we will simply load the json file using json.load() function.
...
The derivative of the ReLU function. In practice the derivative at x = 0 can be set to either 0 or 1. The zero derivative for negative x can give rise to problems when training a neural network, since a neuron can become 'trapped' in the zero region and backpropagation will never change its weights. [2]
